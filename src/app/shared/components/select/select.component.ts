...
export class ZardSelectComponent implements ControlValueAccessor, AfterContentInit, OnDestroy {
  ...
  private readonly syncSelectItemsEffect = effect(() => {
    this.syncSelectItems(this.selectItems());
  });
  ...
  private syncSelectItems(items: readonly ZardSelectItemComponent[]): void {
    const hostWidth = this.elementRef.nativeElement.offsetWidth || 0;
    const compactMode = hostWidth <= COMPACT_MODE_WIDTH_THRESHOLD;
    this.isCompact.set(compactMode);
    ...
    items.forEach((item, index) => {
      ...
      item.zMode.set(compactMode ? 'compact' : 'normal');
    });
  }
